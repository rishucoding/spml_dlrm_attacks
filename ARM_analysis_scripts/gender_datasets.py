...
print(buy_log['final_gender_code'].unique())

genders = buy_log['final_gender_code'].unique()

for gender in genders:
    buy_log = buy_log1
    buy_log = buy_log[buy_log['final_gender_code'] == gender]
    buy_log = buy_log.drop(
        columns=['cms_segid', 'cms_group_id', 'age_level', 'pvalue_level', 'shopping_level', 'occupation',
    ...
    # map gender code to actual gender
    gender_code = 'male' if gender == 1 else 'female'

    # Save the modified dataset to a new CSV file
    df.to_csv(f'../dataset/taobao_dataset_with_orders_{gender_code}.csv', index=False)
